...
poetry run python manage.py initadmin >> django_init.txt || true # allow this command to fail with set -e
...
poetry run python manage.py loaddata delete_mothly.json
